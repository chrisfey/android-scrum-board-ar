...
public class ImageProcessing {
    ...
    public static final String LOGTAG = App.LOGTAG + "ImageProcessing";
    ...
    private static Mat hsvToBrightness(Mat imageHsv) {
        ArrayList<Mat> hsv = new ArrayList<>(3);
        Core.split(imageHsv, hsv);
        return hsv.get(2);
    }
    ...
    private static ArrayList<SortedMap<Integer, Mat>> extractDigits(ContourResult postItNotes, Mat hsvImage) {
        ...
        for (int i = 0; i < postItNotes.points.size(); i++) {
            ...
            Mat bound = hsvImage.submat(digitRect);
    ...
    private static Mat threshold(Mat src, int thresh, int maxVal) {
        ...
        src.copyTo(dst);
        Mat output = new Mat();
        Imgproc.threshold(dst, output, thresh, maxVal, Imgproc.THRESH_OTSU);
        return output;
    }
